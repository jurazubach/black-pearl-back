...
Object.values(LISTEN_FILTERS).forEach((filterKey) =>
  Object.assign(listenFilterSchema, filterSchema(filterKey))
);
...
export const Filters = createParamDecorator(
  (data: unknown, ctx: ExecutionContext): IFilters => {
    ...
    try {
      const parsedFilters: IFilters = parseQueryFilters(
      ...
      throw new HttpException(
        "Can not correct parse passed filters",
