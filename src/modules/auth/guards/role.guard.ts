...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const requireRole = this.reflector.get<USER_ROLE>('requireRole', context.getHandler());

    const req = context.switchToHttp().getRequest();
    const auth = req.auth as IJwtToken;
    const userRole = auth.role || USER_ROLE.USER;

    const accessRoles = ROLE_HIERARCHY[requireRole] || ROLE_HIERARCHY[USER_ROLE.USER];
