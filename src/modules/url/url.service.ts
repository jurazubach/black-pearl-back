...
export class UrlService {
  ...
  getFrontBaseUrl(path?: string, queryParams?: { [key: string]: any }) {
    ...
    const stringifyQueryParams = queryParams ? `${delimited}${qs.stringify(queryParams)}` : '';

    return `${protocol}://${host}${port ? `:${port}` : ''}${path || ''}${stringifyQueryParams}`;
  }
  ...
  getApiBaseUrl(path?: string, queryParams?: { [key: string]: any }) {
    ...
    return `${protocol}://${host}${port ? `:${port}` : ''}/api${path}${stringifyQueryParams}`;
  }
}
