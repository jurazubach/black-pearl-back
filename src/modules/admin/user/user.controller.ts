import { Controller, Get, Put, Delete, HttpCode, HttpStatus, Body, HttpException, Res } from '@nestjs/common';
...
export class UserController {
  ...
  async putMe(@Body() payload: PutUserDTO, @Auth() auth: IJwtToken) {
    ...
    await this.userService.updateUser(user.id, pick(user, ['firstName', 'lastName', 'lang']));
  ...
  @AuthGuard()
  @HttpCode(HttpStatus.OK)
  async putChangePassword(@Body() payload: ChangePasswordDTO, @Auth() auth: IJwtToken) {
  ...
  @ApiBearerAuth('token')
  @HttpCode(HttpStatus.OK)
  async deleteMe(@Auth() auth: IJwtToken, @Res({ passthrough: true }) res: Response) {
    const user = await this.userService.getSecureUserByParams({
      email: auth.email,
    });
